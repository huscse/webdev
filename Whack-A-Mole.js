...
function StartRealTimer(startTime){
    ...
    if(!isPaused){
        let interval = setInterval(() => {
            ...
            actualTimer.innerHTML = `${parseInt((totalGameTime+1)-elapsed.toFixed(1))}`;
            ...
            if (elapsed > totalGameTime) {
                ...
                finalScore.innerHTML = "FINAL SCORE: "+document.getElementById("ActualScore").innerHTML;
            ...
            for (let d = 0; d <amountOfMolesAtOnce; d++){
                ...
                document.getElementById("gameBoard").addEventListener('click',MoleMissed);
...
function PopUpMole(molee){
    ...
    //Which row and which hole to pick
    var randomRow = parseInt(Math.random()*3+1);
    var randomHole = parseInt(Math.random()*3+1);

    //style the molee to be at the bottom of the hole and have the ability to get 10 points (classList.add("Up"))
    ...
    setTimeout(() => {
        //Actually move the molee up
         molee.style.bottom = "50px"; 
    }, 10);
...
function PopDownMole(molee){
    ...
    //This is called when the game wants the mole to be down, so move the molee down and classify the lower amount of points (classList.add("Down"))
...
function MoleClickedGood(event){
    ...
    document.getElementById("ActualScore").innerHTML = Number(document.getElementById("ActualScore").innerHTML)+amountofPoints;
}
